        ...
        <li key={song.id}>
          ...
          >
            ...
            <span className="min-w-0">
              <span className="block truncate text-base font-extrabold leading-snug tracking-[-0.02em]">{song.title}</span>
              <span className="mt-0.5 block truncate text-sm text-muted-foreground">{song.artist}</span>
              <span className="mt-1 block text-xs text-muted-foreground">{formatSongSource(song.source)}</span>
            </span>
          </button>
        </li>
      ))}
